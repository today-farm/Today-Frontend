...
import {  ComponentWrapper,Error,Success,BottomButton } from './../../../style/CommonStyles';
...
function Account() {
  ...
  useEffect(() => {
      axios.get(`/user/find-my-info`,{ headers: { Authorization: `Bearer ${cookies.accessToken}` }})
      .then((res) => {
        setInfo((prev) => ({
          ...prev,
  ...
  const handleDoubleCheck = async () => {
    ...
    return axios.post(`/sign-up/nickname-duplicate-check`, formData, {
  ...
  const handleUpdate = async () => {
    ...
    if(changeNickname){
      await formData.append(
        ...
        new Blob([JSON.stringify(changeNickname)], { type: 'application/json' })
      );
    }
    return axios.patch(`/user/update-my-info`, formData, {
  ...
  return (
    <ComponentWrapper>
        ...
        <NickNameInput defaultValue={info.nickname} onChange={handleChangeNickname}/>
        <Error>{error}</Error>
