...
import { ComponentWrapper, Inputs, InputWrapper,Label,Input, BottomButton,Error } from './../../../style/CommonStyles';
...
function ChangePassword() {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
   if (e.target.name === 'changePassword') {
      ...
      if (!Checkpassword(e.target.value)) {
        setError((prev) => ({
          ...prev,
          changePasswordCheckError: '비밀번호가 일치하지 않아요! 다시 확인해 주세요!',
        }));
      } else {
        setError((prev) => ({
          ...prev,
  ...
  const handleUpdate = async () => {
    ...
    if(password.currentPassword ==="" || password.changePassword === "" || password.changePasswordCheck){
      alert("빠진 정보가 없는지 확인해 주세요!")
    }
    return axios.patch(`/user/update-password`, formData, {
    ...
    .catch(err=>{
      ...
      if(err.response.data.errorCode === 2005){
        setError((prev) => ({
          ...prev,
          currentPasswordError: '비밀번호가 잘못되었어요! 다시 확인해 주세요.',
        }));       
      }
    })
  };
  ...
  return (
    <ComponentWrapper>
      ...
      <Inputs height={240}>
        <InputWrapper>
          <Label>현재 비밀번호</Label>
          <Input type='password' name="currentPassword" placeholder='현재 비밀번호를 입력해 주세요.' onChange={handleCurrentPassword}/>
          <Error>{error.currentPasswordError}</Error>
        </InputWrapper>
        <InputWrapper>
          <Label>변경할 비밀번호</Label>
          <Input type='password' name="changePassword" placeholder='새로운 비밀번호를 입력해 주세요.' onChange={handleChange}/>
          <Error>{error.changePasswordError}</Error>
        </InputWrapper>
        <InputWrapper>
          <Label>비밀번호 확인</Label>
          <Input type='password' name="changePasswordCheck" placeholder='비밀번호를 한 번 더 입력해 주세요.'onChange={handleChange} />
